...
module.exports = {
  ...
  run: async ( client, message, [parameter]) => {
    ...
    if (parameter && parameter.toLowerCase() === 'reload'){
      ...
      if (!memes.size){
        return message.channel.send('<:cancel:767062250279927818> | Could not fetch memes from <:reddit:767062345422864394> [Reddit](https://reddit.com/r/animemes)! Please report this to the bot owner. The API might be down or there might be changes on the API itself.');
      };
      ...
      if (!memes.size) {
        return message.channel.send(
          ...
          .setFooter(`Animeme | \©️${new Date().getFullYear()} 𝕯𝖗𝖆𝖌𝖔𝖓𝖇𝖔𝖞#6241`)
          .setDescription(
