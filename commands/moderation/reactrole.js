  ...
  run: async (client, message, args, settings) => {
    ...
    if (args[0].toLowerCase() === 'embed') {
        const rrCreateDescription = new MessageEmbed()
            ...
            if(!message.member.hasPermission('MANAGE_GUILD'))return message.channel.send(`${client.configg.emojis.error}Vous devez avoir la permission de gérer les channels pour utiliser cette commande.`);
        if(settings.reactionroles){
             try{ 
            const guild = settings
            const channel = client.resolveChannel(message.channel.guild, args[1]);
            if (!channel) return message.channel.send(`${client.configg.emojis.FALSE} Je n'ai pas trouver ce channel.`);
            ...
            if (!messageV) return message.channel.send(`${client.configg.emojis.FALSE} Je n'ai pas trouver ce message.`);
            let emote = await client.resolveGuildEmoji(message.channel.guild, args[3].trim());
            if (!emote && client.isUnicode(args[3])) emote = args[3];
            if (!emote) return message.channel.send(`${client.configg.emojis.FALSE} Je n'ai pas trouver cet emoji.`);
            args.splice(0, 4);
            const role = client.resolveRole(message.channel.guild, args.join(' '));
            if (!role || role.id == message.channel.guild.id) return message.channel.send(`${client.configg.emojis.FALSE} Impossible de trouver ce rôle.`);
            ...
            if(existingReactionRole)return message.channel.send(`${client.configg.emojis.FALSE} Cet émoji est déja associé a un role sous ce message.`);
            //.find(r => r.emoji == emote && r.messageID == message.id);
            //if (existingReactionRole) return message.channel.send(`${client.configg.emojis.FALSE} Il  y a déja un role associé a cet emoji sous ce message.`);
        ...
        // await client.updateGuild(message.guild, { reactionroles: [{channelID: channel.id, messageID: messageV.id, emoji: emote.id ? emote.id : emote, roleID: role.id }]});
            ...
            message.channel.send(`${client.configg.emojis.TRUE}J'ai bien crée un role-reaction sous ce message.`);
        /* const newGuild = await delta.db.Guild.findOne({ ID: msg.channel.guild.id });
            ...
            return msg.channel.createMessage({ embed });*/
                ...
                if(e.message.match('Unknown Message'))return message.channel.send(`${client.configg.emojis.error}Je n'ai pas trouver ce message.`);
                else return message.channel.send(`${client.configg.emojis.error}Une erreur s'est produite. Merci de vérifiez les paramètres de la commande.`);
            }
        }else{
            return message.channel.send(`${client.configg.emojis.FALSE}Une erreur s'est produite merci de réessayer.`)
        }
    }
    if(args[0].toLowerCase() === 'rem'){
        ...
        if (args.length == 2 && args[1] == 'all') {
            ...
            guild.save();
            return message.channel.send(`${client.configg.emojis.TRUE}Tous les roles-reactions du serveur ont bien été supprimés`);
        }else{
            try{
            ...
            if (!messageV) return message.channel.send(`${client.configg.emojis.FALSE} Je n'ai pas trouver ce message.`);
            if (!guild.reactionroles.find(r => r.messageID === messageV.id)) return message.channel.send(`${client.configg.emojis.FALSE} Il n'y a pas de role-reaction sous ce message.`);
            let emote = await client.resolveGuildEmoji(message.channel.guild, args[3].trim());
            if (!emote && client.isUnicode(args[3])) emote = args[3];
            if (!emote) return message.channel.send(`${client.configg.emojis.FALSE} Je n'ai pas trouver cet emoji.`);
            args.splice(0, 4);
            ...
            return message.channel.send(`${client.configg.emojis.TRUE} J'ai bien supprimer ce role reaction.`);
           // console.log(del)
            }catch(e){
