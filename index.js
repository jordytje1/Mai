...
client.on('message', message => {
    ...
    if(message.author.bot) {
        if(message.embeds.length === 1 && message.embeds[0].description.startsWith('React')) {
            message.react(':white_check_mark:823906739333890098')
...
client.on('raw', payload => {
    if(payload.t === 'MESSAGE_REACTION_ADD') { // Check if the event name is MESSAGE_REACTION_ADD
        ...
        {
            if(payload.d.message_id === '823906739333890098') { // Here we check if the id of the message is the ID of the embed that we had the bot send using the ?sendmsg command.
                ...
                if(channel.messages.has(payload.d.message_id)) { // Check if the channel has the message in the cache.
                    ...
                    .then(msg => {
                        let reaction = msg.reactions.get('white_check_mark:823906739333890098');
        ...
        else if(payload.d.emoji.name === 'white_check_mark') {
...
client.on('messageReactionAdd', (reaction, user) => {
    if(reaction.emoji.name === 'white_check_mark') { // If the emoji name is ticketreact, we will create the ticket channel.
        /**
